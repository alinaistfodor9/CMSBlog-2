(function (speak) {
    speak.pageCode(["/-/speak/v1/formsbuilder/assets/formservices.js"],
        function (formServices) {
            var formsPageAppUrl = "/Applications/FormsBuilder/Pages/Forms",
                ...
                removeVersionConfirmationMessage = "",
                messageTypes = {
                    loadForm: "LoadForm",
            ...
            return {
                initialized: function () {
                    ...
                    this.on({
                        ...
                        "form:RemoveVersion": this.removeVersion,
                        "RemoveVersionConfirmationDialog:ButtonClick": this.confirmRemoveVersionClicked,
                    ...
                    removeVersionConfirmationMessage = this.RemoveVersionConfirmationDialog.Message;
                    ...
                    var lang = _sc.Helpers.url.getQueryParameters(window.location.href)["lang"];
                    this.FormDesignBoard.loadForm(lang);
                    ...
                    this.SaveFormSubAppRenderer.on("saveform:ProceedSave", this.saveFormProceedSave, this);
                ...
                selectLanguageBeforeChanged: function (obj) {
                    ...
                    if (this.FormDesignBoard.CurrentLanguage !== value &&
                        (this.FormDesignBoard.HasChanged || this.PropertyGrid.hasChanged())) {
                        obj.isSelectionAborted = true;
                        this.confirmUnappliedThenUnsavedChanges(
                            this.proceedSelectLanguage.bind(this, obj.item[this.LanguageListControl.ValueFieldName]));
                    }
                },
                ...
                loadFormCompleted: function (formModel) {
                    if (this.FormDesignBoard.FormMode !== "new") {
                        formServices.getImportantNotes(this.FormDesignBoard.FormId, this.FormDesignBoard.FormMode, this.FormDesignBoard.CurrentLanguage).
                            then(this.getImportantNotesSuccess.bind(this)).fail(this.getImportantNotesFail.bind(this));
                    }
                    ...
                    this.ActionControl.getAction("RemoveVersion").IsDisabled =
                        this.FormDesignBoard.FormMode !== "edit" ||
                        !this.LanguageListControl.SelectedItem ||
                        !this.LanguageListControl.SelectedItem.hasVersions;
                    this.ActionControl.getAction("Delete").IsDisabled = !this.SelectedItem;
                ...
                getImportantNotesSuccess: function (response) {
                    this.removeOldMessage("GetImportantNotes");

                    if (response) {
                        this.MessageBar.add({
                            MessageId: "GetImportantNotes",
                            Type: "warning",
                            Text: response,
                            IsClosable: true
                        });
                ...
                getImportantNotesFail: function (response) {
                    this.removeOldMessage("GetImportantNotes");
                ...
                saveFormSuccess: function (saveOptions, data) {
                    this.removeOldMessage("PublishForm");
                    this.removeOldMessage("SaveForm");

                    var replaceStateOptions = {
                        lang: this.FormDesignBoard.CurrentLanguage
                    };
                    ...
                    if (saveOptions.editExisting) {
                        ...
                        if (formId) {
                            replaceStateOptions.formId = formId;
                            replaceStateOptions["sc_formmode"] = "edit";
                    ...
                    var urlQuery = speak.Helpers.url.addQueryParameters(window.location.search, replaceStateOptions);
                ...
                saveFormProceedSave: function (formName, parentId) {
                    this.saveOptions.formName = formName;
                    this.saveOptions.parentId = parentId;

                    this.proceedSaveForm();
                },

                showSaveDialog: function (dialogText) {
                    this.SaveFormSubAppRenderer.show(this.FormDesignBoard.FormModel.model.name,
                        this.FormDesignBoard.FormModel.parentId,
                        dialogText);
                },
                ...
                saveFormAs: function () {
                    ...
                    this.confirmUnappliedFieldChanges(this.showSaveDialog.bind(this));
                },
                ...
                saveFormAsTemplateOrForm: function () {
                    ...
                    this.confirmUnappliedFieldChanges(this.showSaveDialog.bind(this, dialogText));
                },
                ...
                publishForm: function () {
                    ...
                    var hasForm = this.FormDesignBoard.FormModel && this.FormDesignBoard.FormModel.model && this.FormDesignBoard.FormModel.model.name;
                    if (hasForm && !this.FormDesignBoard.HasChanged) {

                        formServices.getPublishingTargets()
                            .then(function (data) {
                                if (data && data.length) {
                                    return data.map(t => t.itemId);
                                }
                            }).then(function (publishingTargets) {
                                var payload = {
                                    publishMode: "Full",
                                    publishingTargetIds: publishingTargets,
                                    forms: [{ "formId": this.FormDesignBoard.FormModel.itemId, "formLanguage": this.FormDesignBoard.FormModel.currentLanguage }]
                                };
                                formServices.publishForm(payload).then(function (data) {

                                    if (data && data.SuccessPublishedForms.length > 0) {
                                        var message = {
                                            MessageId: "PublishForm",
                                            Type: "notification",
                                            Text: this.PublishSuccessMessage.Text,
                                            IsClosable: true
                                        };
                                    ...
                                    var message = {
                                        MessageId: "PublishForm",
                                        Type: "error",
                                        Text: this.PublishErrorMessage.Text,
                                        IsClosable: true
                                    };
                                    this.MessageBar.add(message);
                        ...
                        var message = {
                            MessageId: "PublishForm",
                            Type: "error",
                            Text: this.SaveBeforePublish.Text,
                            IsClosable: true
                        };
                        this.MessageBar.add(message);
                    }
                ...
                removeVersionCompleted: function (removeStatus) {
                    this.removeOldMessage("RemoveVersion");

                    var message = {
                        MessageId: "RemoveVersion",
                        Type: "warning",
                        Text: "",
                        IsClosable: true
                    };
                    ...
                    switch (removeStatus) {
                        case "success":
                            ...
                            message.Text = this.RemoveVersionSuccessMessage.Text;
                            debouncedLoadForm.call(this);
                            break;

                        case "noaccess":
                            message.Text = this.RemoveVersionNoAccessMessage.Text;
                            break;

                        case "nootherversions":
                            message.Text = this.RemoveVersionNoOtherVersionMessage.Text;
                            break;

                        default:
                            message.Type = "error";
                            message.Text = this.RemoveVersionErrorMessage.Text;
                            break;
                    }

                    message.Text =
                        speak.Helpers.string.format(message.Text, this.LanguageListControl.SelectedItem.displayName);
                    this.MessageBar.add(message);
                },

                removeVersionError: function (xhr) {
                    this.removeVersionCompleted();
                },

                proceedRemoveVersion: function () {
                    var dialogText = removeVersionConfirmationMessage;
                    dialogText =
                        speak.Helpers.string.format(dialogText, this.LanguageListControl.SelectedItem.displayName);

                    this.RemoveVersionConfirmationDialog.Message = dialogText;
                    this.RemoveVersionConfirmationDialog.show();
                },

                removeVersion: function () {
                    this.confirmUnappliedThenUnsavedChanges(this.proceedRemoveVersion);
                },

                confirmRemoveVersionClicked: function (buttonControlId) {
                    if (buttonControlId[0] !== "ok")
                        return;

                    formServices
                        .removeVersion(this.FormDesignBoard.FormModel.itemId, this.FormDesignBoard.CurrentLanguage)
                        .then(this.removeVersionCompleted.bind(this)).fail(this.removeVersionError.bind(this));
                },
